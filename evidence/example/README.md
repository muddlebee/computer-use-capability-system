...
New runs contain JSONL events, an automatically persisted `result.json`, an `index.html` gallery, a masked start screenshot, and one masked screenshot after every step. Discovery also includes `model-tool-calls.jsonl` with structured, safe-to-persist model actions and rationales. The handoff directory also contains the typed intervention request and the screenshot presented to the operator.
